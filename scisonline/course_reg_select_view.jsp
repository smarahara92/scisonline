<%-- 
    Document   : course_reg_select_view
--%>
...
<%
        ...
        String s = "";
        int i = 0;
        System.out.println("given Stream "+stream +" " + given_session + " " + given_year);
%>
        <table>
            <tr>
...
<%
        ...
        if (!(stream == null)&& !(stream.equalsIgnoreCase("Other")) ) {
            ...
            rs = (ResultSet) st2.executeQuery("select Code, Subject_Name , " + stream + ", type from subjecttable as a," + given_year + "_" + given_session + "_elective as b where a.Code=b.course_name and b." + stream + "!=0");
%>
            <table>
                <tr>
                    <td>&nbsp;</td>
        ...
        $( "select" ).change(function() { 
            ...
            if(stream!==null) {
                if(stream==="")
                    ...
                    var hint="yes";
                    // alert(stream);
                    parent.left.location="./course_reg_select_view.jsp?stream="+stream+"&given_session=<%=given_session%>&given_year=<%=given_year%> ";
                    //stream = stream.replace('-', '_');
                    parent.act_area.location="./choose_ele_other_schools_view.jsp?stream=Other&given_session=<%=given_session%>&given_year=<%=given_year%> ";
                } else {   //stream = stream.replace('-', '_');
                    parent.left.location="./course_reg_select_view.jsp?stream="+stream+"&given_session=<%=given_session%>&given_year=<%=given_year%> ";
                    //stream = stream.replace('-', '_');
                    parent.act_area.location="./choose_ele_view.jsp?stream="+stream+"&given_session=<%=given_session%>&given_year=<%=given_year%> ";
                }
            }
